...
window.WIRE_MAP = {
  'behind-dash': {
    ...
    shotList: '',
    pins: [
      { id: 'fuse-panel',   part: 'AAW Fuse Panel',            x: 310,  y: 625, verdict: 'connect', checks: ['bat-panel'],
        info: 'AAW 500707 seated in the stock bulkhead hole, firewall driver side — both screws tight, no pinched wires. Battery + ignition feeds in via the bulkhead; 18 fused circuits out. HDX taps GAUGES (PINK) + CLOCK (RED); the fuel sender is unfused. Flasher can mounts bottom-right corner of the panel.' },
      { id: 'column',       part: 'Steering Column (clamp area)', x: 650, y: 500, verdict: 'connect',
        info: 'Column clamp bracket at the dash (3/8"-16, 20 ft-lb). The turn-signal switch connector plugs in at the column base — sheet #17, 7-wire plus horn BLACK. The horn button grounds through the column, so the horn relay BLACK depends on this path.' },
      { id: 'hdx-box',      part: 'HDX Control Box',           x: 1170, y: 385, verdict: 'connect',
        info: 'Behind dash, driver side — accessible for testing, away from HEI/coil. In: RED const 12V (CLOCK fuse), PINK sw 12V (GAUGES fuse), ORANGE dimmer, all sender leads. Out: 8-pin display cable (~3 ft) to the cluster + BLACK dedicated ground. Ignition EMI is the dominant display-cable failure mode — mount away from the coil (MAN 650542H p.4).' },
      { id: 'heater-box',   part: 'Heater Box',                x: 1380, y: 420, verdict: 'connect',
        info: 'Cabin-side heater distribution case (non-A/C), passenger side — houses the blower feed and the resistor, which mounts ON the box in the airflow. The #21 resistor and #24 heater-switch plugs land nearby. Only HIGH works later = resistor burned out.' },
      { id: 'ground-bolt',  part: 'Dedicated Ground Bolt',     x: 975,  y: 355, verdict: 'connect', checks: ['gnd-hdx'],
        info: '5/16"-18 bolt + star washer on the firewall/dash brace, driver side — bare metal. Takes the HDX Control Box BLACK (18 AWG, no splices) to chassis → battery negative. Must read < 0.5 Ω to battery negative — never share it with other accessories.' },
      { id: 'bulkhead',     part: 'Bulkhead Connector',        x: 450,  y: 355, verdict: 'connect',
        info: 'AAW 510105 on the firewall, driver side, ~6" left of center. Engine-bay harness (Bag J) in, dash harness (Bag G) out. Seal with dielectric grease on the terminals + RTV on the cavities — it’s the weather barrier.' },
      { id: 'flasher',      part: 'Turn-Signal Flasher',       x: 400,  y: 710, verdict: 'connect',
        info: '2-pin can at the fuse panel bottom-right corner. Fuse panel TURN feed in, column turn-signal switch out. Signals stay solid instead of flashing? Replace this can. (The hazard can is a separate flasher — position per the printed install sheet.)' },
      { id: 'headlight-sw', part: 'Headlight Switch + Dim Kit', x: 300, y: 280, verdict: 'connect', checks: ['chs-headlight'],
        info: 'AAW 500332 in the stock dash location, driver side. Park/headlight feeds in; the 520001 dim-kit GREY dimmed feed → HDX ORANGE (backlight dimming). The rotary knob rheostat drives the HDX backlight level.' },
    ],
    routes: [
  ...
  'engine-bay': {
    ...
    pins: [
      { id: 'oil-sender',  part: 'Oil Pressure Sender',       x: 895,  y: 540, verdict: 'connect', checks: ['hdx-oil'],
        info: 'SEN-03-8 in the block oil-galley port, driver side, mid-height (where the factory sender was) — 1/8" NPT, Teflon tape, hand-tight + ¼ turn. All four conductors landed: WHITE sig / RED 5V / BLACK gnd + bare shield (22 AWG), Dakota sub-harness direct to Control Box. Route the lead 3-4" from headers (heat loom if tight).' },
      { id: 'temp-sender', part: 'Coolant Temp Sender',       x: 605,  y: 528, verdict: 'connect', checks: ['hdx-temp'],
        info: 'SEN-04-5 in the intake manifold water jacket, passenger-side front — 1/8" NPT via a 3/8"→1/8" reducing bushing. Two wires, BOTH direct to Control Box TEMP INPUT-SIG + TEMP INPUT-GND — the ground does NOT go through the block. Drain coolant below the port first; 45°/90° elbow if clearance is tight.' },
      { id: 'tach-source', part: 'Tach Signal Source',        x: 700,  y: 200, verdict: 'connect', checks: ['hei-tach'],
        info: 'HEI: the TACH terminal on the cap (points: coil negative (–) instead). One WHITE 18 AWG Dakota lead direct to the Control Box terminal marked TACH. Keep it ≥6" from plug wires — suppression wires only.' },
      { id: 'alternator',  part: 'Alternator + Ground Strap', x: 440,  y: 620, verdict: 'connect', checks: ['alt-charge', 'alt-ground'],
        info: '6 ga RED charge wire on the alternator BAT stud — boot on, nut tight — to the battery via Bag Z, protected by the MEGAFUSE. The engine-to-chassis ground strap (block → frame/firewall) is the alternator’s return path — essential for stable gauges; missing strap = low/erratic voltmeter + gauge jitter.' },
      { id: 'megafuse',    part: 'Megafuse / Main Power',     x: 90,   y: 750, verdict: 'connect', checks: ['bat-megafuse'],
        info: 'Bag Z 510476 assembly under hood, close to the battery source — bussbar jumper D on the starter/battery-feed side (the #1 Bag Z assembly error), notched cover on, all four M8 nuts + lock washers snug. Battery/starter post in; main feed out to the fuse panel via the bulkhead. Protects the whole harness — mount before routing the main feed.' },
      { id: 'eb-bulkhead', part: 'Bulkhead Connector',        x: 875,  y: 250, verdict: 'connect',
        info: 'AAW 510105 on the firewall, driver side (under the master cylinder). Engine-bay harness (Bag J) in, dash harness (Bag G) out; the 10 ga RED main feed passes through here to the fuse panel BAT bus. Dielectric grease on the terminals + RTV on the cavities — it’s the weather barrier.' },
    ],
    routes: [
  ...
  'under-car': {
    ...
    pins: [
      { id: 'vss-port',     part: 'Speed Sensor Adapter', x: 1290, y: 440, verdict: 'connect', checks: ['hdx-speed'],
        info: 'Cable-drive adapter (SEN-01-5 type) threads into the stock speedo-cable port on the transmission tailshaft. Seat it fully — it slips out under vibration (the classic no-speedo cause). PURPLE sig / RED 5V / BLACK gnd (22 AWG), Dakota sub-harness direct to Control Box. Calibrate after install.' },
      { id: 'frame-ground', part: 'Frame/Body Ground',    x: 600,  y: 490, verdict: 'connect', checks: ['gnd-body-frame'],
        info: 'Frame-to-body ground bolt, driver side — chassis return paths to battery negative. Clean to bare metal during recommission; part of the < 0.5 Ω ground chain. AAW Step 3: body→frame AND battery→frame grounds must both exist before the battery goes in.' },
    ],
    routes: [
  ...
  'rear': {
    ...
    pins: [
      { id: 'tank-sender', part: 'Fuel Tank Sending Unit', x: 585, y: 285, verdict: 'connect', checks: ['fuel-boot', 'fuel-pair'],
        info: 'Factory 0–90 Ω unit inside the tank, top access — no replacement needed, the HDX calibrates to it. TAN lead rubber boot seated on the sender stud, wire clear of exhaust/filler, up through the trunk-floor feed hole. Twisted pair with ground stays twisted full length to the HDX FUEL INPUT (unfused); ground wire on a sender mounting screw. Reads backwards later → Invert setting, don’t rewire.' },
      { id: 'rear-ground', part: 'Rear Body Ground',       x: 130, y: 765, verdict: 'connect', checks: ['gnd-rear'],
        info: 'Bag M ring terminal on bare trunk-area metal, rear body panel. Tail-light + fuel-sender returns to chassis. A bad rear ground makes tail lights dim AND the fuel gauge wander.' },
    ],
    routes: [
  ...
  'aaw-sheet': {
    ...
    routes: [],
    pins: [
      { id: 'aaw-1',  part: 'Dimmer Switch',                  x: 1263, y: 562, verdict: 'connect', checks: ['chs-dimmer'],
      ...
      { id: 'aaw-4',  part: 'Rear Body',                      x: 1370, y: 585, verdict: 'connect', checks: ['chs-rear'],
      ...
      { id: 'aaw-5',  part: 'Instrument Cluster Disconnects', x: 1525, y: 120, verdict: 'harvest', checks: ['hdx-ind'],
      ...
      { id: 'aaw-6',  part: 'Wiper',                          x: 1280, y: 185, verdict: 'connect', checks: ['chs-wiper'],
      ...
      { id: 'aaw-7',  part: 'Headlight Switch',               x: 1352, y: 150, verdict: 'connect', checks: ['chs-headlight'],
      ...
      { id: 'aaw-8',  part: 'Brake Switch',                   x: 1768, y: 590, verdict: 'connect', checks: ['chs-brake'],
      ...
      { id: 'aaw-9',  part: 'Ground',                         x: 1709, y: 262, verdict: 'connect', checks: ['gnd-dash'],
        info: 'Dash BLACK ground to bare chassis metal. Keep SEPARATE from ground #26 — never combine.' },
      { id: 'aaw-10', part: 'Horn Relay',                     x: 1516, y: 508, verdict: 'connect', checks: ['chs-horn'],
      ...
      { id: 'aaw-12', part: 'Turbo 400 Kickdown Feed',        x: 1457, y: 545, verdict: 'connect', checks: ['chs-kickdown'],
      ...
      { id: 'aaw-13', part: 'Neutral Safety Switch',          x: 1930, y: 112, verdict: 'connect', checks: ['chs-nss'],
        info: 'Console shifter NSS: PURPLE in / PURPLE out → starter solenoid S. No NSS = no crank. Cranks in P/N only.' },
      { id: 'aaw-14', part: 'Back Up Switch',                 x: 1996, y: 106, verdict: 'connect', checks: ['chs-nss'],
      ...
      { id: 'aaw-17', part: 'Turn Signal Switch',             x: 1830, y: 113, verdict: 'connect', checks: ['chs-column'],
      ...
      { id: 'aaw-20', part: 'RH Courtesy Lamp',               x: 2350, y: 508, verdict: 'cap', checks: ['chs-capped'],
        info: 'No interior courtesy lamps on this build — cap and label.' },
      { id: 'aaw-21', part: 'Heater Resistor',                x: 2427, y: 150, verdict: 'connect', checks: ['chs-heater'],
      ...
      { id: 'aaw-22', part: 'Glove Box',                      x: 2265, y: 133, verdict: 'cap', checks: ['chs-capped'],
      ...
      { id: 'aaw-24', part: 'Heater Switch',                  x: 2160, y: 120, verdict: 'connect', checks: ['chs-heater'],
      ...
      { id: 'aaw-26', part: 'Ground',                         x: 1692, y: 568, verdict: 'connect', checks: ['gnd-dash'],
