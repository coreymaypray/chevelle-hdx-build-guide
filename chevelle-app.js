...
const DASH_GAUGES = [
  { key: 'tach', n: 'Tachometer', range: '0 – 8000 RPM', cx: 88, cy: 88, r: 58, tft: { x: 58, y: 156, w: 60, h: 14 }, color: '#d8d8e0',
    ...
    wire: 'WHITE 18 AWG — AAW Connector H',
    cal: 'Set cylinder count to 8 (396/454)',
    fuse: null,
    trace: ['Coil (–) / HEI TACH', 'Firewall grommet', 'AAW Connector H (WHITE)', 'HDX Control Box TACH', 'Cluster — RPM'],
  ...
  { key: 'speed', n: 'Speedometer', range: '0 – 140 MPH', cx: 220, cy: 88, r: 58, tft: { x: 190, y: 156, w: 60, h: 14 }, color: '#9b59d0',
    ...
    fuse: null,
    trace: ['Trans tailshaft adapter', 'Floor grommet', 'AAW VSS connector', 'HDX Control Box SPEED', 'Cluster — MPH'],
  ...
  { key: 'oil', n: 'Oil Pressure', range: '0 – 100 PSI', cx: 330, cy: 58, r: 26, color: '#d8d8e0',
    ...
    fuse: null,
    trace: ['SEN-03-8 at block port', 'Firewall grommet', 'AAW Connector F', 'HDX Control Box OIL', 'Cluster — PSI'],
  ...
  { key: 'temp', n: 'Water Temp', range: '100 – 260 °F', cx: 388, cy: 58, r: 26, color: '#1f8a4c',
    ...
    wire: 'DARK GREEN signal 18 AWG — AAW Connector G',
    cal: 'None — factory calibrated',
    fuse: null,
    trace: ['SEN-04-5 at intake port', 'Firewall grommet', 'AAW Connector G (DK GREEN)', 'HDX Control Box TEMP', 'Cluster — °F'],
    note: 'Grounds through the engine block — no separate ground wire. Missing the 3/8"-to-1/8" NPT reducer bushing = no seal = coolant leak. Drain coolant below the port before pulling the plug.' },
...
const CIRCUITS = [
  ...
  { id: 'tach', name: 'Tachometer Signal', swatch: '#d8d8e0',
    hdxWire: 'WHITE', aawWire: 'WHITE via Connector H', awg: '18 AWG',
    fuse: null, hotWhen: 'Engine running (signal)', aawRef: 'AAW Connector H',
    route: ['Coil (–) [points] / HEI TACH terminal', 'Firewall grommet (≥6" from plug wires)', 'AAW Connector H', 'HDX Control Box TACH'],
  ...
  { id: 'oil', name: 'Oil Pressure Sender', swatch: '#e0a13a',
    hdxWire: 'WHITE sig / RED 5V / BLACK gnd + bare shield', aawWire: 'via Connector F', awg: '22 AWG sensor wire',
    fuse: null, hotWhen: '5V reference from Control Box', aawRef: 'AAW Connector F · Dakota SEN-03-8',
    route: ['SEN-03-8 at block oil-galley port (driver side, 1/8" NPT)', 'Away from headers (3-4" min, heat loom)', 'Firewall grommet', 'AAW Connector F', 'HDX Control Box OIL'],
  ...
  { id: 'temp', name: 'Water Temp Sender', swatch: '#1f8a4c',
    hdxWire: 'DARK GREEN signal', aawWire: 'via Connector G', awg: '18 AWG',
    fuse: null, hotWhen: 'Signal only — grounds through block', aawRef: 'AAW Connector G · Dakota SEN-04-5',
    route: ['SEN-04-5 at intake manifold water jacket (passenger front)', '3/8"→1/8" NPT reducing bushing', 'Firewall grommet', 'AAW Connector G', 'HDX Control Box TEMP'],
    warnings: ['Missing the 3/8"-to-1/8" NPT reducer bushing = no seal = coolant leak = engine damage. Verify bushing fit first.'],
    notes: ['2-wire system: grounds through the engine block, no separate ground wire. If reading is wrong, check sender metal-to-metal contact.'] },
  { id: 'speed', name: 'Speed Sensor (VSS)', swatch: '#9b59d0',
    hdxWire: 'PURPLE sig / RED 5V / BLACK gnd', aawWire: 'AAW VSS connector', awg: '22 AWG sensor wire',
    fuse: null, hotWhen: '5V reference from Control Box', aawRef: 'Cable-drive adapter at trans tailshaft',
    route: ['Adapter threads into tailshaft (where the mechanical cable was)', 'Floor/firewall grommet', 'AAW VSS connector', 'HDX Control Box SPEED'],
  ...
  { zone: 'Engine Bay', icon: 'engine',
    ...
    parts: [
      { n: 'Oil Pressure Sender', pn: 'SEN-03-8', loc: 'Block oil-galley port, driver side, mid-height (where the factory sender was)', thread: '1/8" NPT',
        wiresIn: '—', wiresOut: 'WHITE sig / RED 5V / BLACK gnd + bare shield (22 AWG) → Connector F',
      ...
      { n: 'Coolant Temp Sender', pn: 'SEN-04-5', loc: 'Intake manifold water jacket, passenger-side front', thread: '1/8" NPT via 3/8"→1/8" reducing bushing',
        wiresIn: '—', wiresOut: 'DARK GREEN signal (18 AWG) → Connector G; grounds through block',
      ...
      { n: 'Tach Signal Source', pn: '—', loc: 'HEI: TACH terminal on cap · Points: coil negative (–)', thread: '—',
        wiresIn: '—', wiresOut: 'WHITE 18 AWG → Connector H', note: 'Keep ≥6" from plug wires. Suppression wires only.' },
  ...
  { zone: 'Transmission & Under Car', icon: 'route',
    parts: [
      { n: 'Speed Sensor Adapter', pn: 'Cable-drive (SEN-01-5 type)', loc: 'Transmission tailshaft — threads into the stock speedo-cable port', thread: 'Cable-port thread',
        wiresIn: '—', wiresOut: 'PURPLE sig / RED 5V / BLACK gnd (22 AWG) → AAW VSS connector',
...
const ENGINE_PARTS = [
  { n: 'Oil Pressure Sender — SEN-03-8', loc: 'Block oil-galley port, driver side', thread: '1/8" NPT', wire: 'WHITE/RED/BLACK + shield → Conn F', note: 'Teflon tape, ¼ turn past snug. Old GM single-wire sender will NOT work. Route away from headers.' },
  { n: 'Coolant Temp Sender — SEN-04-5', loc: 'Intake manifold, passenger front', thread: '3/8"→1/8" NPT bushing', wire: 'DK GREEN → Conn G (grounds through block)', note: 'Drain coolant below the port first. Verify the reducer bushing before installing.' },
  { n: 'Speed Sender — cable-drive', loc: 'Trans tailshaft', thread: 'Adapter into cable port', wire: 'PURPLE/RED/BLACK → AAW VSS conn', note: 'Seat fully — slips out under vibration. GPS-calibrate after install.' },
  { n: 'Tach Signal', loc: 'HEI TACH terminal / coil (–)', thread: '—', wire: 'WHITE → Conn H', note: 'Keep 6 inches from plug wires; suppression wires mandatory.' },
...
const TROUBLE = [
  ...
  { icon: '⑩', title: 'Ammeter vs Voltmeter Confusion',
    ...
    fix: 'Disconnect and insulate the old ammeter bypass wire (heavy-gauge wire through the firewall). The HDX voltmeter needs only its signal wire on the battery/charging circuit.',
